...
// BLE設定
#define SERVICE_UUID        "4fafc201-1fb5-459e-8fcc-c5c9c331914b" // transmitter用サービスUUID
#define CHARACTERISTIC_UUID "beb5483e-36e1-4688-b7f5-ea07361b26a8" // transmitter用キャラクタリスティック

#define SERVICE_UUID_TANAKA_GATE        "e1e2e3e4-1111-2222-3333-444455556666" // 新しいtanaka_gate用サービスUUID
#define CHARACTERISTIC_UUID_TANAKA_GATE "e1e2e3e4-aaaa-bbbb-cccc-ddddeeeeffff" // 新しいtanaka_gate用キャラクタリスティックUUID
...
// BLE関連変数
BLEServer* pServerTransmitter = NULL;
BLECharacteristic* pCharacteristicTransmitter = NULL;
BLEServer* pServerTanakaGate = NULL;
BLECharacteristic* pCharacteristicTanakaGate = NULL;
bool deviceConnectedTransmitter = false;
bool oldDeviceConnectedTransmitter = false;
bool deviceConnectedTanakaGate = false;
bool oldDeviceConnectedTanakaGate = false;
...
// BLE接続状態管理コールバッククラス（transmitter用）
class MyServerCallbacksTransmitter: public BLEServerCallbacks {
    void onConnect(BLEServer* pServer) {
      deviceConnectedTransmitter = true;
      Serial.println("*** BLE client connected (transmitter) ***");
    };

    void onDisconnect(BLEServer* pServer) {
      deviceConnectedTransmitter = false;
      Serial.println("*** BLE client disconnected (transmitter) ***");
    }
};
// BLE接続状態管理コールバッククラス（tanaka_gate用）
class MyServerCallbacksTanakaGate: public BLEServerCallbacks {
    void onConnect(BLEServer* pServer) {
      deviceConnectedTanakaGate = true;
      Serial.println("*** BLE client connected (tanaka_gate) ***");
    };

    void onDisconnect(BLEServer* pServer) {
      deviceConnectedTanakaGate = false;
      Serial.println("*** BLE client disconnected (tanaka_gate) ***");
    }
};
